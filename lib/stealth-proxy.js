...
function filterXmppClientTraffic(content) {
...
function rewriteClientConfig(jsonText, localChatPort) {
    try {
        ...
        if (data['chat.affinities'] && typeof data['chat.affinities'] === 'object') {
            for (const key of Object.keys(data['chat.affinities'])) {
                ...
                if (typeof affHost === 'string' && affHost) {
                    ...
                    data['chat.affinities'][key] = '127.0.0.1';
                }
            }
        }
...
async function startStealthProxy(options = {}) {
    ...
    chatServer.on('secureConnection', (clientSocket) => {
        ...
        clientSocket.on('data', (chunk) => {
            ...
            const filteredStr = filterXmppClientTraffic(str);
    ...
    const configServer = http.createServer((req, res) => {
        ...
        const proxyReq = https.request({
            ...
            proxyRes.on('end', () => {
                ...
                if (contentType.includes('application/json') || body.toString('utf8').trim().startsWith('{')) {
                    const text = body.toString('utf8');
                    const { modifiedJson, upstreamHost, upstreamPort } = rewriteClientConfig(text, chatPort);
                    
                    if (upstreamHost && upstreamHost !== '127.0.0.1') {
                    ...
                    const outBuffer = Buffer.from(modifiedJson, 'utf8');
                    const headers = { ...proxyRes.headers, 'content-length': outBuffer.length };
