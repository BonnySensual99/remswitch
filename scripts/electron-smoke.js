...
async function main() {
    ...
    const accountsView = await windowRef.webContents.executeJavaScript("({ view: document.body.dataset.view, title: document.getElementById('pageTitle').textContent, briefHidden: getComputedStyle(document.getElementById('dashboardBrief')).display === 'none' })");
    await windowRef.webContents.executeJavaScript("document.querySelector('[data-tab=\\\"dashboard\\\"]').click()");
    const dashboardView = await windowRef.webContents.executeJavaScript("({ view: document.body.dataset.view, briefVisible: getComputedStyle(document.getElementById('dashboardBrief')).display !== 'none' })");
    ...
    if (!apiShape.hasApi || !apiShape.hasRendererApiCollision || !apiShape.addButton || !apiShape.accountCard || !apiShape.gameBadge || !apiShape.currentBadge || accountsView.view !== 'accounts' || accountsView.title !== 'Bóveda de cuentas' || !accountsView.briefHidden || dashboardView.view !== 'dashboard' || !dashboardView.briefVisible || !accountModalOpen || !settingsModalOpen) {
