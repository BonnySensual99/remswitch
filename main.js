...
async function startAccountSwitch(accountId, requestId, targetGame = null) {
    ...
    try {
        ...
        if (alreadyLoggedIn) {
            ...
            if (activeSession) {
                emitSwitch({ ...payloadBase, state: 'LoggingOut', message: `Cerrando la sesi�n de ${activeSession.riotId}�` });
            ...
            emitSwitch({ ...payloadBase, state: 'ClosingExistingSession', message: 'Cerrando la sesi�n anterior�' });
...
function updateTrayMenu(sessionInfo = undefined) {
    ...
    const menuTemplate = [
        ...
        {
            label: activeRiotId ? `? Riot: ${activeRiotId}` : '? Riot: Sin sesi�n',
            enabled: false
        },
        { type: 'separator' },
